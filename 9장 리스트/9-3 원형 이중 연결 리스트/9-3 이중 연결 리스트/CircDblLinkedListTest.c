...
typedef enum {
	...
	PRINT_CRNT, RMV_CRNT, SRCH_NO, SRCH_NAME, FIND_N_NODE, RMV_REPEAT, PRINT_ALL, NEXT, PREV, CLEAR
}Menu;
...
Menu SelectMenu()
{
	...
	char* mstring[] = {
		...
		"n번째의 포인터 반환", "중복 노드 삭제", "모든 노드를 출력", 
		"선택한 노드를 뒤쪽으로", "선택한 노드를 앞쪽으로", "모든 노드를 삭제"
	};

	do {
		for (i = TERMINATE; i < CLEAR; i++) {
			printf("(%2d) %-22s ", i + 1, mstring[i]);
			if (i % 3 == 2) putchar('\n');
		}
		printf("( 0) 종료 : ");
		scanf_s("%d", &ch);
